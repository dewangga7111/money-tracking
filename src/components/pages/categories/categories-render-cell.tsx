...
export default function CategoriesRenderCell({ item, columnKey, onDelete }: RenderCellProps) {
  ...
  switch (key) {
    case 'icon':
      return <div className="text-2xl">{cellValue}</div>;

    case 'type':
      return (
        <span
          className={`rounded-full px-2 py-1 text-xs font-semibold ${cellValue === 'INCOME' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
        >
          {cellValue}
        </span>
      );
      ...
      return (
        <Dropdown>
          ...
          <Dropdown.Popover placement="right" className="min-w-32">
            <Dropdown.Menu aria-label="Category actions">
              <Dropdown.Item
                key="edit"
                onAction={() => router.push(`/categories/edit/${item.key}`)}
              >
